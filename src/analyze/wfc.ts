...
*/
const mythXsave = function(object: any) {
    const self = this;
    ...
    return new Promise(function(accept, reject) {
        ...
        fs.readFile(outputPath, {encoding: 'utf8'}, function(err: any, json: string) {
...
*/
function prepareConfig(options: any) {
...
*/
function multiPromisify (func: Function) {
    // FIXME: accumulating this to a list is weird.
    const resultList: any[] = [];
    return (...args: any[]) => new Promise( (accept, reject) => {
...
export async function compile (options: any) {
...
export async function compileSources (config: any, compilers: string[]) {
    return Promise.all(
            ...
            if (results && results.length > 0) {
                ...
                for (const result of results) {
                    ...
                    if (stale) {
                        ...
                        await writeContracts(artifact, config);
                    }
                }
            }
...
export async function writeContracts (artifact: any, options: any) {
