...
export const parseImports = (body: string, solc: any) => {
  ...
  const errors = output.errors.filter(function(solidity_error: any) {
    return solidity_error.message.indexOf(preReleaseCompilerWarning) < 0;
  });

  const nonImportErrors = errors.filter(function(solidity_error: any) {
  ...
  const imports = errors
    .filter(function(solidity_error: any) {
      return solidity_error.message.indexOf(failingImportFileName) < 0;
    })
    .map(function(solidity_error: any) {
