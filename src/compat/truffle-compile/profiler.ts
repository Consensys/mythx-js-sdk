...
export const updated = (options: any, callback: any) => {
  ...
  function getFiles(done: Function) {
    if (options.files) {
  ...
  const sourceFilesArtifacts: any = {};
  const sourceFilesArtifactsUpdatedTimes: any = {};

  const updatedFiles: Array<any> = [];

  async.series(
    [
      // Get all the source files and create an object out of them.
      function(c) {
        getFiles(function(err: any, files: Array<string>) {
          if (err) {
            return c(err);
          }
      ...
      function(c) {
        ...
        Object.keys(sourceFilesArtifacts).forEach(function(sourceFile) {
          ...
          sourceFilesArtifactsUpdatedTimes[sourceFile] = artifacts.reduce(
            function(minimum: any, current: any) {
        ...
        async.map(
          ...
          function(err, sourceFileStats) {
            ...
            sourceFiles.forEach(function(sourceFile, index) {
              const sourceFileStat: any = sourceFileStats[index];
  ...
  find_contracts(options.contracts_directory, (err: any, allPaths: any) => {
    ...
    supplier
      ...
      .then(solc => {
        ...
        resolveAllSources(resolver, allPaths, solc, (e: any, resolved: any) => {
          if (e) {
            return callback(e);
          }
          ...
          updates.forEach((update: any) => compilationTargets.push(update));
...
export const resolveAllSources = (resolver: any, initialPaths: any, solc: any, callback: Function) => {
  const mapping: any = {};
  const allPaths = initialPaths.slice();

  function generateMapping(finished: any) {
    const promises = [];
    ...
    while (allPaths.length) {
      ...
      const promise = new Promise((accept, reject) => {
        resolver.resolve(file, parent, (err: any, body: any, absolutePath: string, source: string) => {
          err
            ? reject(err)
    ...
    Promise.all(promises)
      .then(results => {
        ...
        results.forEach(
          (item: any) => (mapping[item.file] = Object.assign({}, item)),
        );

        // Queue unknown imports for the next resolver cycle
        while (results.length) {
          const result: any = results.shift();
...
export const getImports = (file: string, resolved: any, solc: any) => {
  ...
  return imports.map((dependencyPath: any) => {
...
export const listsEqual = (listA: any, listB: any) => {
  const a = listA.sort();
  const b = listB.sort();
...
export const convert_to_absolute_paths = (paths: any, base: any) => {
  return paths.map(function(p: string) {
