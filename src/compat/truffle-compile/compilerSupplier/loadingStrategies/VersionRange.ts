...
export default class VersionRange extends LoadingStrategy {
  public compilerFromString(code: string) {
  ...
  public findNewestValidVersion(version: any, allVersions: any) {
  ...
  public getCachedSolcByVersionRange(version: string) {
  ...
  public getCachedSolcFileName(commit: any) {
    const cachedCompilerFileNames = fs.readdirSync(this.cachePath);
  ...
  public getCommitFromVersion(versionString: string) {
    return 'commit.' + versionString.match(/commit\.(.*?)\./)[1];
  }

  public getMostRecentVersionOfCompiler(versions: any) {
    return versions.reduce((mostRecentVersionFileName: string, fileName: string) => {
      const match = fileName.match(/v\d+\.\d+\.\d+.*/);
  ...
  public getSatisfyingVersionFromCache(versionRange: any) {
  ...
  public async getSolcByCommit(commit: string) {
  ...
  public async getSolcByUrlAndCache(fileName: string) {
  ...
  public async getSolcFromCacheOrUrl(version: string) {
    let allVersions;
    try {
  ...
  public getSolcVersionFileName(version: string, allVersions: any) {
  ...
  public async load(versionRange: any) {
  ...
  public normalizeSolcVersion(input: any) {
    const version = String(input);
    return version.split(':')[1].trim();
  }

  public versionIsCached(version: any) {
    const cachedCompilerFileNames = fs.readdirSync(this.cachePath);
