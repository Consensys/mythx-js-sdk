...
export default class LoadingStrategy {
  ...
  public addFileToCache(code: string, fileName: string) {
    const filePath = this.resolveCache(fileName);
    fs.writeFileSync(filePath, code);
  }

  public errors(kind: string, input = '', error = '') {
  ...
  public fileIsCached(fileName: string) {
    const file = this.resolveCache(fileName);
    return fs.existsSync(file);
  }

  public load(_userSpecification: any) {
    throw new Error(
  ...
  public resolveCache(fileName: string) {
