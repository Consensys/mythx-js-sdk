...
function isExplicitlyRelative(import_path: string) {
  return import_path.indexOf('.') === 0;
}

function convertToAbsolutePath(p: string, base: string, nodeBase: string) {
...
const getSourceFileName = (sourcePath: any) => {
...
function sourcePath2BuildPath(sourcePath: string, buildDir: string) {
...
*/
function staleBuildContract (sourcePath: string, buildPath: string) {
...
const cleanBytecode = (bytecode: string) => {
...
const normalizeJsonOutput = (jsonObject: any) => {
...
//   logger: console
// }
const compile = (sourcePath: string, sourceText: string, options: any, callback: any, isStale: boolean) => {
  ...
  supplier
    ...
    .then((solc: any) => {
      ...
      if (options.strict !== true) {
        warnings = errors.filter(function(error: any) {
...
// quiet: Boolean. Suppress output. Defaults to false.
// strict: Boolean. Return compiler warnings as errors. Defaults to false.
const necessary = function(options: any, callback: any) {
  options.logger = options.logger || console;

  Profiler.updated(options, function(err: any, updated: any) {
    if (err) {
      return callback(err);
    }
...
const with_dependencies = (options: any, callback: any, compileAll: boolean) => {
  options.logger = options.logger || console;
...
const display = (paths: Array<string>, options: any) => {
